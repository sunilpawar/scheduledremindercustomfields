...
/**
 * Extended Membership ActionMapping with Custom Field Support
 */
class CRM_Scheduledremindercustomfields_ActionMapping_Member extends CRM_Member_ActionMapping {
  ...
  /**
   * Add custom field JOINs and WHERE conditions
   */
  ...
  /**
   * Add JOIN for custom field table
   */
  protected function addCustomFieldJoin($query, $entity, $fieldInfo, $joinIndex) {
    ...
    switch ($entity) {
      ...
      case 'Activity':
        // For membership reminders, we might need to join activities differently
  ...
  /**
   * Add WHERE condition for custom field
   */
  protected function addCustomFieldWhere($query, $joinAlias, $fieldInfo, $operator, $value, $joinIndex) {
    ...
    $paramName = "customField{$joinIndex}";
      ...
      case 'NOT IN':
        if (is_array($value) && !empty($value)) {
          ...
          foreach ($value as $val) {
            ...
            $paramIndex += 100; // Avoid conflicts
          }
